...
manifest |> 
  filter(Disease == "Vasculitis") |> arrange(Age) |> View()
```
...
hpa <- read_tsv("data/hpa_v25.tsv")

selected_proteins <- 
  dat |> 
  filter(!is.na(Significance),
        Significance != "Not significant in both")

classes <- hpa |> 
  select(Gene, `Protein class`) |> 
  separate_rows(`Protein class`, sep = ", ") |> 
  inner_join(selected_proteins, by = c("Gene" = "Assay")) |> 
  count(Significance, `Protein class`) 
...
classes |> 
  left_join(top_classes) |> 
  filter(`Protein class` %in% top_classes$`Protein class`) |> 
  arrange(-n) |> 
  ggplot(aes(fct_reorder(`Protein class`, -total_n), n, fill = Significance)) +
  geom_col(show.legend = F) +
    scale_fill_manual(values = de_platform_pal) +
  theme_hpa(angled = T) +
  xlab("Protein class")

ggsave("results/Manuscript-figures/bamse_protein_classes.pdf", h = 5, w = 6)
