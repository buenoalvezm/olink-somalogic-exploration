...
# Compute % above LOD for each combined protein-aptamer pair
pair_lod_summary <- olink |>
  inner_join(soma, by = c("DAid", "Assay")) |>
  group_by(Assay, AptName) |>
  summarise(
    pct_above_lod = mean((NPX > LOD) & (Value > eLOD), na.rm = TRUE) * 100,
    .groups = "drop"
  )

lod_bin_summary <- correlation |>
  select(Assay, AptName, correlation) |>
  inner_join(pair_lod_summary, by = c("Assay", "AptName")) |>
  mutate(
    lod_bin = pmin(floor(pct_above_lod / 10) * 10, 100)
  ) |>
  group_by(lod_bin) |>
  summarise(
    count = n(),
    median_spearman = median(correlation, na.rm = TRUE),
    .groups = "drop"
  ) |>
  complete(
    lod_bin = seq(0, 100, by = 10),
    fill = list(count = 0, median_spearman = NA_real_)
  )

max_count_lod <- max(lod_bin_summary$count, na.rm = TRUE)

lod_performance_plot <- ggplot(lod_bin_summary, aes(x = lod_bin)) +
  geom_col(aes(y = count, fill = "Count"), width = 8, alpha = 0.6) +
  geom_line(
    aes(y = median_spearman * max_count_lod, color = "Median Spearman", group = 1),
    linewidth = 0.8
  ) +
  geom_point(
    aes(y = median_spearman * max_count_lod, color = "Median Spearman"),
    size = 1.6
  ) +
  scale_x_continuous(
  ...
  scale_y_continuous(
    name = "Count",
    sec.axis = sec_axis(~ . / max_count_lod, name = "Median Spearman Correlation")
  ) +
  scale_fill_manual(values = c("Count" = "#66B2A3"), name = NULL) +
  scale_color_manual(values = c("Median Spearman" = "#3F3F7F"), name = NULL) +
  labs(
    x = "Above LOD (%)",
    y = "# Protein-Aptamer",
    tag = "C"
...
# Add Panel C in the same row, keeping A->B orientation unchanged
top_combined_with_lod <- global_cor_plot_secretome_no_legend + arrow_plot +
  max_cor_plot_secretome_no_legend + lod_performance_plot +
  plot_layout(ncol = 4, widths = c(1, 0.1, 1, 1)) +
  plot_annotation(theme = theme(plot.margin = margin(t = 5, r = 5, b = 0, l = 5)))

# Add legend at bottom
final_combined_plot <- top_combined_with_lod / legend +
...
protein_plots[[1]][[1]] <- protein_plots[[1]][[1]] + labs(tag = "D")
...
boxplot_combined[[1]] <- boxplot_combined[[1]] + labs(tag = "E")
boxplot_combined[[2]] <- boxplot_combined[[2]] + labs(tag = "F")
...
corr_by_disease_plot <- corr_by_disease_plot + labs(tag = "G")
...
## Intra-SomaScan Correlations

```{r}
apt_counts <- soma |> count(Assay, AptName) |> count(Assay, name = "n_apt") |> filter(n_apt > 1)
multi_assays <- apt_counts$Assay

soma_multi <- soma |> filter(Assay %in% multi_assays)

cor_results <- lapply(multi_assays, function(assay) {
  dat <- soma_multi |> filter(Assay == assay) |> select(DAid, AptName, Value)
  wide <- dat |> pivot_wider(names_from = AptName, values_from = Value)
  aptnames <- setdiff(names(wide), "DAid")
  if (length(aptnames) < 2) return(NULL)
  combs <- combn(aptnames, 2, simplify = FALSE)
  lapply(combs, function(pair) {
    vals <- wide |> select(all_of(pair))
    cor_val <- suppressWarnings(cor(vals[[1]], vals[[2]], method = "spearman", use = "complete.obs"))
    data.frame(Assay = assay, AptName1 = pair[1], AptName2 = pair[2], correlation = cor_val)
  }) |> bind_rows()
}) |> bind_rows()
...
ggplot(cor_results, aes(x = correlation)) +
  ...
  labs(
    x = "Intra-SomaScan Spearman correlations",
    y = "# Somamers"
  ) +
  theme_hd() +
  theme(
...
ggsave(
  filename = "results/intra_somascan_correlations.png",
  width = 8,
  height = 6,
  units = "cm"
)
...
cor_results_max <- cor_results |>
  group_by(AptName1) |>
  slice_max(order_by = correlation, n = 1, with_ties = FALSE) |>
  ungroup()

scatter_data <- correlation |>
  inner_join(cor_results_max, by = c("AptName" = "AptName1")) |>
  select(AptName, olink_soma = correlation.x, max_intra_soma = correlation.y)

p <- ggplot(scatter_data, aes(x = olink_soma, y = max_intra_soma)) +
  geom_point(alpha = 0.9, color = "lightblue") +
  labs(
    x = "Olink-SomaScan correlation",
    y = "Max intra-SomaScan correlation",
  ) +
  theme_hd() +
  theme(axis.text.x = element_text(hjust = 0.5))

p_marginal <- ggMarginal(p, type = "histogram", bins = 30, fill = "lightblue")

ggsave("results/olink_soma_intrasoma_scatter.png", p_marginal, width = 10, height = 10, units = "cm", dpi = 300, bg = "white")
```
...
soma_all <- soma_all |> filter(DAid %in% common_daids)

rm(soma_wide)

protein <- "CRP"

soma_protein <- soma_all |>
  filter(Assay == protein) |>
  group_by(DAid, Assay) |>
  summarise(Value = mean(Value, na.rm = TRUE), .groups = "drop") |>
  mutate(Technology = "SomaScan")

if (nrow(soma_protein) == 0) {
  stop(paste0("No rows available for assay '", protein, "' in soma_all."))
}

plot_data <- soma_protein |>
  inner_join(
    meta |>
      select(DAid, Disease, Class) |>
      mutate(Disease = if_else(stringr::str_starts(Disease, "BAMSE"), "BAMSE", Disease)),
    by = "DAid"
  )

if (nrow(plot_data) == 0) {
  stop(paste0("No rows available for assay '", protein, "' after joining metadata."))
}
...
highlight_disease <- if (protein == "CRP") {
  c(
    "Healthy",
    "Longevity",
    "Acute COVID",
...
combined_data_filtered <- plot_data |>
  mutate(
    highlight = Disease %in% highlight_disease,
    edge_color = ifelse(highlight, "#000000", "#00000003"),
    point_color = ifelse(highlight, "#000000", "#d3d3d3")
...
color_bar <- ggplot(
  ...
  geom_tile() +
  scale_fill_manual(values = disease_class, drop = FALSE) +
  theme_void() +
  theme(
    legend.position = "none",
    plot.margin = margin(t = 0, r = 5, b = 0, l = 5)
...
boxplotwith_colorbar <- boxplot / color_bar +
plot_layout(heights = c(10, 0.2))
boxplotwith_colorbar

ggsave(
  filename = "results/CRP_somascan_plot.pdf",
  width = 10,
  height = 4,
  units = "cm"
)
