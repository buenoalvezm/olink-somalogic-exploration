...
summary_stats <- bind_rows(correlation_results_disease) |>
  ...
  left_join(meta |> select(Disease, Class) |> distinct(), by = "Disease") |>
  mutate(
    Disease = factor(Disease, levels = levels(combined_data_filtered$Disease)),
...
combined_corr_plot <- rescued_bar_plot / corr_by_disease_plot +
  plot_layout(heights = c(0.3, 1))

# Update the final plot assembly
top_grob <- patchworkGrob(final_combined_plot)
bottom_grob <- patchworkGrob(combined_bottom)
combined_corr_grob <- patchworkGrob(combined_corr_plot)

final_plot <- grid.arrange(
  top_grob,
  bottom_grob,
  combined_corr_grob,
  heights = c(1, length(protein_plots), 2)
)
