...
# Step 3: Create the upper scatterplot
upper_plot <- ggplot(scatter_data, aes(x = NPX, y = Value)) +
  geom_point(aes(color = Class), alpha = 0.6) +
  geom_smooth(method = "auto", se = TRUE, color = "black") +
  geom_text(
    data = label_df,
  ...
  scale_color_manual(values = disease_class) +
  facet_grid(Class ~ Assay, scales = "free") +
  labs(x = "Olink (NPX)", y = "SomaScan (log2RFU)") +
  theme_hd() +
  theme(
...
# Step 5: Create the lower boxplot
lower_plot <- ggplot(combined_data, aes(x = Disease, y = Value, fill = Class)) +
  geom_boxplot(outlier.shape = NA, alpha = 1) +
  #geom_jitter(position = position_jitterdodge(jitter.width = 0.2), size = 0.5, alpha = 0.6) +
  facet_grid(Technology ~ Assay, scales = "free") +
...
```{r}
combined_plot <- upper_plot / lower_plot + plot_layout(heights = c(6, 2))
...
ggsave("results/combined_plot_A4.png", combined_plot, width = 21, height = 29.7, units = "cm")
```

REVERSED

```{r}
# Step 1: Identify the class with the best correlation for each Assay
best_class_per_assay <- scatter_data |>
  filter(Class != "Global") |>  # Exclude Global for this calculation
  group_by(Assay) |>
  summarise(
    best_class = Class[which.max(rho)],  # Find the class with the highest correlation
    .groups = "drop"
  )

# Step 2: Filter scatter_data to include only Global and the best class per Assay
filtered_scatter_data <- scatter_data |>
  inner_join(best_class_per_assay, by = "Assay") |>  # Join to get the best class
  filter(Class == "Global" | Class == best_class) |>  # Keep only Global and the best class
  mutate(
    Facet_Class = if_else(Class == "Global", "Global", "Best Class"),  # Create a new column for faceting
    Facet_Class = factor(Facet_Class, levels = c("Global", "Best Class"))  # Ensure Global comes first
  )

# Step 3: Update label_df to match the filtered scatter_data
filtered_label_df <- label_df |>
  inner_join(best_class_per_assay, by = "Assay") |>  # Join to get the best class
  filter(Class == "Global" | Class == best_class) |>  # Keep only Global and the best class
  mutate(
    Facet_Class = if_else(Class == "Global", "Global", "Best Class"),  # Create a new column for faceting
...
# Step 4: Create the updated upper plot
upper_plot_r <- ggplot(filtered_scatter_data, aes(x = NPX, y = Value)) +
  geom_point(aes(color = Class), alpha = 0.6) +  # Color by original class
  geom_smooth(method = "auto", se = TRUE, color = "black") +
  geom_text(
    data = filtered_label_df,
    aes(x = -Inf, y = Inf, label = label),
    hjust = -0.05, vjust = 1.2, size = 3
  ) +
  scale_color_manual(values = disease_class) +  # Use the original class colors
  facet_grid(Assay ~ Facet_Class, scales = "free") +  # Facet by Global and Best Class
  labs(x = "Olink (NPX)", y = "SomaScan (log2RFU)") +
  theme_hd() +
  theme(
    legend.position = "None",
...
```{r}
# Step 5: Create the lower boxplot
lower_plot_r <- ggplot(combined_data, aes(y = Disease, x = Value, fill = Class)) +
  geom_boxplot(outlier.shape = NA, alpha = 1) +
  #geom_jitter(position = position_jitterdodge(jitter.width = 0.2), size = 0.5, alpha = 0.6) +
  facet_grid(Assay ~ Technology, scales = "free") +
  labs(x = "Disease", y = "Value") +
  scale_fill_manual(values = disease_class) +
  theme_hd() +
  theme(
...
```{r}
combined_plot <- upper_plot_r + lower_plot_r + plot_layout(widths = c(5, 5))
...
ggsave("results/combined_plot_A4_r_sub.png", combined_plot, width = 21, height = 29.7, units = "cm")
```