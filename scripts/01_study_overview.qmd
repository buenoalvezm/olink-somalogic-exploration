...
## Number of samples and diseases
...
meta |> distinct(DAid) |> nrow()
data_olink |> distinct(DAid) |> nrow()
common_samples|> distinct(DAid) |> nrow()


# Disease + healthy (n)
individuals_non_longitudinal <- 
  common_samples|> 
  filter(!Cohort %in% c("BAMS", "PREG")) |> 
  nrow()
...
meta_pregnancy <- 
    manifest |> 
    filter(Cohort == "PREG") |> 
    separate(`Vial barcode`, into = c("Subject", "Trimester"), sep = "_", remove = FALSE) |> 
    select(DAid, Subject, Age, Sex, BMI, Trimester)

individuals_pregnancy <- 
  common_samples|> 
  filter(Cohort == "PREG") |> 
  distinct(DAid) |> 
  left_join(meta_pregnancy) |> 
  count(Subject) |> 
  arrange(n) |> 
  #count(Trimester) |> 
  nrow() # 6 pregnancy samples just 2 visits

# BAMSE (n)
individuals_bamse <- 
  meta_bamse |> 
  filter(DAid %in% common_samples$DAid) |> 
  count(Subject) |> 
  arrange(n) |> 
  nrow()

individuals_non_longitudinal + individuals_bamse + individuals_pregnancy
```
...
# List number of assays for manuscript
overlap_aptamer_level <- 
  meta_soma |> 
  distinct(AptName, EntrezGeneSymbol)  |>
  filter(EntrezGeneSymbol %in% ht_assays) |> 
  pull(AptName)


assay_summary <- tibble(
  dataset = c("Olink", "SomaScan", "Overlap", "Overlap - aptamer level"),
  n_assays = c(
    length(ht_assays),
    length(soma_assays),
    length(overlapping_assays),
    length(overlap_aptamer_level)
  )
)
...
data_soma |> 
  ...
  arrange(-n_aptamers) |> 
  mutate(
n_aptamers = if_else(n_aptamers >= 4, "4+", as.character(n_aptamers)),
    n_aptamers = factor(n_aptamers, levels = c("1", "2", "3", "4+"))
  ) |> 
...
saveRDS(umap_olink, "data/processed/umap_olink.rds")
...
saveRDS(umap_soma, "data/processed/umap_soma.rds")
